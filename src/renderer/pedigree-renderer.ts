...
export class PedigreeRenderer {
  ...
  private validateDataset(): string[] {
    ...
    for (const ind of this.dataset) {
      if (ind.mother && !this.individuals.has(ind.mother)) {
        errors.push(`Mother '${ind.mother}' not found for '${ind.name}'`);
      }
      if (ind.father && !this.individuals.has(ind.father)) {
        errors.push(`Father '${ind.father}' not found for '${ind.name}'`);
      }
      if (ind.mother) {
  ...
  private calculateGenerations(): Map<string, number> {
    ...
    const queue: { name: string; gen: number }[] = founders.map((f) => ({ name: f.name, gen: 0 }));
      ...
      for (const ind of this.dataset) {
        if (ind.mother === name || ind.father === name) {
          if (!generations.has(ind.name)) {
            const motherGen = ind.mother ? generations.get(ind.mother) : gen;
            const fatherGen = ind.father ? generations.get(ind.father) : gen;
            if (motherGen !== undefined && fatherGen !== undefined) {
              queue.push({ name: ind.name, gen: Math.max(motherGen, fatherGen) + 1 });
            }
          }
        }
      }
  ...
  private calculatePositions(): void {
    ...
    const numGenerations = Math.max(...Array.from(generations.values())) + 1;
    const verticalSpacing = (height - padding * 2) / Math.max(numGenerations - 1, 1);

    // Calculate required width based on largest generation
    const maxGenSize = Math.max(...Array.from(genGroups.values()).map((g) => g.length));
    ...
    for (const [gen, individuals] of genGroups) {
      ...
      const evenSpacing = (effectiveWidth - padding * 2) / Math.max(individuals.length, 1);
      ...
      sorted.forEach((ind, idx) => {
        ...
        this.nodePositions.set(ind.name, { individual: ind, x, y, generation: gen });
      });
    }
  ...
  renderSvg(): string {
    ...
    const { width, height, symbol_size, background, node_background, font_size, font_family } =
      this.options;
    ...
    // Draw individuals
    this.drawIndividuals(svg as any, symbol_size, node_background, font_family, font_size);
  ...
  private drawFamilyLines(
    ...
    for (const partnership of this.partnerships) {
      ...
      const consanguineous = isConsanguineous(ind1, ind2, this.individuals);
      ...
      if (partnership.children.length > 0) {
        ...
        for (const childPos of childPositions) {
          const lineX = childPositions.length === 1 ? midX : childPos.x;
          drawLine(svg, lineX, sibshipY, childPos.x, childPos.y - symbolSize / 2);
        }
          ...
          if (child.mztwin && !processedTwins.has(child.mztwin)) {
            ...
            if (twins.length > 0) {
              const allTwinPositions = [
                childPos,
                ...twins.map((t) => this.nodePositions.get(t.name)!).filter(Boolean),
              ];
              const twinMinX = Math.min(...allTwinPositions.map((p) => p.x));
              const twinMaxX = Math.max(...allTwinPositions.map((p) => p.x));
              const twinBarY = (sibshipY + (childPos.y - symbolSize / 2)) / 2;
  ...
  private drawIndividuals(
    ...
    for (const [, pos] of this.nodePositions) {
      ...
      drawLabel(g as any, labelText, labelY, fontFamily, fontSize, '#333');
      labelY += lineHeight;

      // Age/YOB
      const showAge = this.options.labels.includes('age') && ind.age !== undefined;
      const showYob = this.options.labels.includes('yob') && ind.yob !== undefined;
      if (showAge || showYob) {
        const parts: string[] = [];
        if (showAge) parts.push(`${ind.age}y`);
        if (showYob) parts.push(`${ind.yob}`);
        drawLabel(g as any, parts.join(' '), labelY, fontFamily, fontSize);
        labelY += lineHeight;
      }
      ...
      if (geneTestLabels.length > 0) {
        drawLabel(g as any, geneTestLabels.join(' '), labelY, fontFamily, fontSize);
      }
    }
  }
}
