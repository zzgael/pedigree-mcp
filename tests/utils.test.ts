...
import { getAncestors, isConsanguineous, getTwins } from '../src/renderer/utils.js';
import type { Individual } from '../src/types.js';

describe('utils', () => {
  describe('getAncestors', () => {
    it('should return empty array for individual with no parents', () => {
      const dataset: Individual[] = [{ name: 'solo', sex: 'M', top_level: true }];
    ...
    it('should return parents and self', () => {
      ...
      expect(ancestors.map((a) => a.name).sort()).toEqual(['child', 'gf', 'gm']);
    });
    ...
    it('should return all ancestors in multi-generation pedigree', () => {
      ...
      expect(ancestors.map((a) => a.name).sort()).toEqual(['child', 'gf', 'ggf', 'ggm', 'gm']);
    });
    ...
    it('should return empty for adopted individuals (noparents flag)', () => {
      ...
      const dataset: Individual[] = [
        ...
        { name: 'adopted', sex: 'F', mother: 'bio_gm', father: 'bio_gf', noparents: true },
      ];
      const individuals = new Map(dataset.map((d) => [d.name, d]));
  ...
  describe('isConsanguineous', () => {
    it('should return false for unrelated partners', () => {
      ...
      const result = isConsanguineous(dataset[0], dataset[1], individuals);
    ...
    it('should return true for first cousins', () => {
      ...
      const dataset: Individual[] = [
        ...
        { name: 'cousin1', sex: 'M', mother: 'aunt_wife', father: 'uncle' }, // Child of uncle
        { name: 'cousin2', sex: 'F', mother: 'mom_wife', father: 'dad' }, // Child of dad
      ];
      const individuals = new Map(dataset.map((d) => [d.name, d]));
    ...
    it('should return false when adoption breaks ancestry chain', () => {
      const dataset: Individual[] = [
        { name: 'ggf', sex: 'M', top_level: true },
        { name: 'ggm', sex: 'F', top_level: true },
        { name: 'adopted', sex: 'M', mother: 'ggm', father: 'ggf', noparents: true },
  ...
  describe('getTwins', () => {
    ...
    it('should return multiple twins (triplets)', () => {
      const dataset: Individual[] = [
        { name: 'f', sex: 'M', top_level: true },
        { name: 'm', sex: 'F', top_level: true },
        { name: 't1', sex: 'F', mother: 'm', father: 'f', mztwin: 'triplets' },
        { name: 't2', sex: 'F', mother: 'm', father: 'f', mztwin: 'triplets' },
        { name: 't3', sex: 'F', mother: 'm', father: 'f', mztwin: 'triplets' },
      ];
    ...
    it('should not match siblings from different parents with same mztwin value', () => {
      const dataset: Individual[] = [
        ...
        { name: 't1', sex: 'F', mother: 'm1', father: 'f1', mztwin: 'same_marker' },
        { name: 't2', sex: 'F', mother: 'm2', father: 'f2', mztwin: 'same_marker' },
      ];
